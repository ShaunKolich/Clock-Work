...
public class UserController {
		...
		if (req.getParameter("userName") != null && req.getParameter("password") != null) {
			try {
				...
				resp.getWriter().println(
						new ObjectMapper().writeValueAsString(user.verifyAndGetUser(req.getParameter("userName") , req.getParameter("password"), resp ) ));
				resp.setStatus(201);
	...
	public void postUser(HttpServletRequest req, HttpServletResponse resp) throws JsonParseException, JsonMappingException, IOException {
		...
		resp.getWriter().println((new ObjectMapper().writeValueAsString( user.verifyAndGetUserByUserObj(new ObjectMapper().readValue(req.getInputStream(), User.class))) ) );
